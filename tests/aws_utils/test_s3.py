...
import boto3
from dagster_io_managers.aws_utils.s3 import create_bucket, get_bucketpath, get_filepath
...
def test_basepath(aws_credentials):
    ...
    assert get_bucketpath(bucket_name=bucket_name) == expected_path
...
def test_filepath(aws_credentials):
    ...
    assert get_filepath(bucket_name=bucket_name, key=key) == expected_path
